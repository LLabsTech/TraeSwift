...
class TraeAgent: BaseAgent {
    ...
    **Guiding Principle:** Act like a senior software engineer. Prioritize correctness, safety, and high-quality, test-driven development.
    ...
    init(config: FullConfig, llmClient: LLMClient) throws {
        ...
        super.init(
            name: "TraeAgent",
            systemPrompt: TraeAgent.traeSystemPrompt,
            tools: tools,
            llmClient: llmClient,
            config: config
        )
    ...
    static func create(from config: FullConfig) throws -> TraeAgent {
        let llmClient = try LLMClientFactory.createClient(from: config)
        return try TraeAgent(config: config, llmClient: llmClient)
    }
}